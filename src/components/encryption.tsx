...
function AutoExpandingTextarea({
  ...
  return (
    <textarea
      ...
      disabled={disabled}
      className="w-full p-2 border rounded-lg dark:bg-zinc-800 dark:border-zinc-700 resize-none overflow-hidden disabled:opacity-50"
  ...
  const handleKeyDown = async (e: KeyboardEvent<HTMLTextAreaElement>) => {
    if (e.key === 'Enter' && !e.shiftKey) {
      ...
      if (text.includes('@ollama')) {
        ...
        try {
          const prompt = text.replace('@ollama', '').trim();
          const generatedText = await generateWithOllama(prompt);
          setPlainText(generatedText);
        } catch (e) {
          console.error('Failed to generate text with AI:', e);
        } finally {
      ...
      description: (
        <div className="space-y-4">
          <div className="relative">
            ...
            />
            {isGenerating && (
              <div className="absolute inset-0 flex items-center justify-center bg-white/80 dark:bg-zinc-800/80 backdrop-blur-sm rounded-lg">
          ...
          <div className="flex items-center gap-2 text-xs text-zinc-500 dark:text-zinc-400">
            <i>Type @ollama followed by your prompt to generate text.</i>
          </div>
        </div>
      ),
    },
